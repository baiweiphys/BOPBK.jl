...
plt = plot(
    ...
    scatter(kk0, wr[:, rootsNo] / abs(wcs[1]),
        ...
        xlabel=L"kρ_{ci}", ylabel=L"\omega_r/\omega_{ci}",
        # xlims=(0, 0.37), 
        ylims=(0, 1),
        grid=true, legend=false),

    # Second subplot: Growth rate (γ/ω_pe)
    scatter(kk0, wi[:, rootsNo] / abs(wcs[1]),
        color=:black, markersize=2,
        xlabel=L"kρ_{ci}", ylabel=L"\gamma/\omega_{ci}",
        # xlims=(0, 0.37), 
        ylims=(-0.05, 0.05),
        grid=true, legend=false), layout=(2, 1),     # 2 rows, 1 column layout
