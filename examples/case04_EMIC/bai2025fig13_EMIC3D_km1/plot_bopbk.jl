...
# idx1 = imag.(ww_contour ./ wps[1]) .> 1.0e1
# idx2 = imag.(ww_contour ./ wps[1]) .< 1.0e-7

idx1 = imag.(ww_contour ./ wcs[1]) .> 5.0e-2
idx2 = imag.(ww_contour ./ wcs[1]) .< 4.0e-3
...
p1 = contourf(kk0, theta_deg, wr_contour[:, :, num_start]' ./ abs(wcs[1]);
...
p2 = contourf(kk0, theta_deg, wi_contour[:, :, num_start]' ./ abs(wcs[1]);
