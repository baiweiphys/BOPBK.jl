...
function maxwell_b34snj(
    ...
    if sgm_val==0
        b34snj = -1.0*bsnj*(In - dIn)
    else
        S3_max(n) = funcS_maxwell(n, lambda_val, sgm_val, 1.0, 1.0, 2.0,EPS0) #S3_mx=S4_mx 
        S8_max(n) = funcS_maxwell(n, lambda_val, sgm_val, 1.0, 1.0, 0.0, EPS0)
        S38_max(n) = S3_max - 2.0*sgm_val*lambda_val*S8_max

        b34snj = bj[jj]*n*wcs[s]*S38_max + bj[jj]*cj[jj]*kz*vtsz[s]*S3_max*Tsx[s]/Tsz[s]/(1+sgm_val)
    end
